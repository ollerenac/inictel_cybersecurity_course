...
**Project:** Curso de Ciberseguridad INICTEL — Wargame Writeups
...
The site is served at an internal IP (192.168.x.x range, INICTEL LAN). These constraints affect stack decisions:

**No CDN access.** All assets must be self-contained in the build output. Docusaurus bundles fonts, icons, and JS locally by default — but verify no external font sources (Google Fonts) leak into the theme config. Set `theme.font = false` or override with a locally-hosted font if needed.

**No external search API.** Install `@cmfcmf/docusaurus-search-local` before the first deploy. Test search by running `npm run build && npx docusaurus serve` and confirming results appear without network access.

**Static files only, no Node.js in production.** Build on a development machine (or CI if INICTEL has one), copy the `/build` directory to the Nginx web root on the server. Nginx reads static files — no Node.js, no npm, no build tooling needed on the serving machine.

**Nginx `baseUrl` must match the serving path.** If the site is at `http://192.168.22.x/curso/`, set `baseUrl: '/curso/'` in `docusaurus.config.js` before building. Wrong `baseUrl` causes broken asset paths and a blank page on first load — this is the most common local deployment failure.

**Port 80 is sufficient.** No TLS certificate is needed for an internal network; Nginx listens on port 80. If the institution later requires HTTPS internally, a self-signed certificate with a local CA root distributed to student browsers is the path — not Let's Encrypt (which requires public DNS).

**Rebuild-to-redeploy workflow.** Since content changes require a rebuild anyway (static site), the deployment workflow is: edit Markdown → `npm run build` → `rsync build/ user@192.168.22.x:/var/www/html/curso/`. This is operationally simple and requires no server-side tooling beyond Nginx.
